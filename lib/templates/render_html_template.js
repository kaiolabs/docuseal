...
async function main() {
  ...
  try {
    ...
    // Get field positions from the rendered DOM (viewport matches A4 @ 96dpi)
    const A4_PAGE_HEIGHT = 1123
    const A4_PAGE_WIDTH = 794

    const fieldData = await page.evaluate((pageHeight, pageWidth) => {
      const pageEl = document.querySelector('.page') || document.body
      const pageRect = pageEl.getBoundingClientRect()
      const contentLeft = pageRect.left + window.scrollX
      const contentWidth = pageRect.width || pageWidth
      ...
      return Array.from(els).map((el) => {
        ...
        return {
          ...
          height: rect.height,
          page: page,
          yOnPage: y - (page * pageHeight),
          contentLeft: contentLeft,
          contentWidth: contentWidth
        }
      })
    }, A4_PAGE_HEIGHT, A4_PAGE_WIDTH)
